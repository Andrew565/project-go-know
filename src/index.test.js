...
/**
 * @param {{numberRank: number, nameRank: string, suit: string}} card
 */
const makeACard = ({numberRank, nameRank, suit}) => {
    return {
        numberRank,
        nameRank,
        suit,
        name: `${nameRank} of ${suit}`,
        value: numberRank,
        initial: nameRank[0],
        facingDown: false
    };
...
describe('Game Logic', () => {
  ...
  test('updateCounters should calculate score correctly', () => {
    ...
    Piles.thresholdActive = makeACard({ numberRank: 10, suit: 'Hearts', nameRank: '10' });
    Piles.obstaclesActive = [makeACard({ numberRank: 5, suit: 'Spades', nameRank: '5' })];

    // Select some cards
    Piles.inventory['i1'].card = makeACard({ numberRank: 5, suit: 'Hearts', nameRank: '5' });
  ...
  test('playCards should move cards to hold if score is sufficient', () => {
     ...
     // Knowble: Value 2
     Piles.thresholdActive = makeACard({ numberRank: 2, suit: 'Clubs', nameRank: '2' });
     Piles.obstaclesActive = [makeACard({ numberRank: 1, suit: 'Spades', nameRank: 'Ace' })]; // Must have obstacle

     // Inventory: Select a card worth 5 (Clubs)
     Piles.inventory['i1'].card = makeACard({ numberRank: 5, suit: 'Clubs', nameRank: '5' });
     Piles.inventory['i1'].selected = true;
     ...
     expect(Piles.hold).toContainEqual(makeACard({ numberRank: 2, suit: 'Clubs', nameRank: '2' }));
     ...
     expect(Piles.inventoryDiscard).toContainEqual(makeACard({ numberRank: 5, suit: 'Clubs', nameRank: '5' }));
     expect(Piles.inventory['i1'].selected).toBe(false);
     expect(Piles.inventory['i1'].card).not.toEqual(makeACard({ numberRank: 5, suit: 'Clubs', nameRank: '5' })); // Should be new card
  });

  test('playCards should discard knowble if score is insufficient', () => {
      // Setup a losing scenario
      Piles.thresholdActive = makeACard({ numberRank: 10, suit: 'Hearts', nameRank: '10' });
      Piles.obstaclesActive = [makeACard({ numberRank: 1, suit: 'Spades', nameRank: 'Ace' })];

      // Select card worth 2 (Hearts)
      Piles.inventory['i1'].card = makeACard({ numberRank: 2, suit: 'Hearts', nameRank: '2' });
      Piles.inventory['i1'].selected = true;

      playCards();

      // Knowble should be in thresholdDiscard
      expect(Piles.thresholdDiscard).toContainEqual(makeACard({ numberRank: 10, suit: 'Hearts', nameRank: '10' }));
      // Inventory card discarded
      expect(Piles.inventoryDiscard).toContainEqual(makeACard({ numberRank: 2, suit: 'Hearts', nameRank: '2' }));
  });

  test('drawObstaclesFor should draw 2 obstacles for King of Clubs', () => {
      // Create a King of Clubs
      const kingClubs = makeACard({ numberRank: 13, suit: 'Clubs', nameRank: 'King' });
  ...
  test('drawObstaclesFor should draw 1 obstacle for non-King of Clubs', () => {
      const queenHearts = makeACard({ numberRank: 12, suit: 'Hearts', nameRank: 'Queen' });
